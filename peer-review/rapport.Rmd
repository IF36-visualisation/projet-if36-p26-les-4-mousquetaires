...
Problématique de la partie: Quelles sont les grands traits du self-made?
...
```{r echo=FALSE}
# Tri des données
dt_finalWorth <- df %>% select(finalWorth) %>% mutate(finalWorth = finalWorth/1000)

# Box plot pour tester la répartition
distr_rich_quart_plot <- dt_finalWorth %>% ggplot(aes(y = 1,x=finalWorth )) +
geom_boxplot() +
  scale_y_continuous(breaks = NULL) +
  labs(title ="Distribution de la richesse des milliardaires",
       x="Milliards") 
...
La richesse des milliardaires est effectivement très concentrée. La médiane est basse et il y a énormément d'outliers, ce qui confirme une distribution très inégale de la richesse parmis les milliardaires. Pour avoir une analyse plus fine, nous proposons de diviser les observations en centiles plutôt qu'en quartiles car le box plot risque d'être encore peu lisible. 
</details>
...
```{r}
# Attribution d'un numéro différent tous les 264 données (2640/10) pour faire une répartition en centile
num_centile <-c("C10", "C09", "C08", "C07","C06", "C05","C04", "C03", "C02", "C01")
dt_centile<- df %>% select(finalWorth) %>% arrange(finalWorth) %>%
  mutate(
...
distr_rich_cent_plot <- dt_centile %>% ggplot(aes(x=factor(centile), y=finalWorth)) +
geom_boxplot()+
  labs(title ="Distribution de la richesse des milliardaires en centile",
       x="Centile",
       y="Milliards")
distr_rich_cent_plot
...
<details>
<summary>Interprétation et ouverture</summary>
Le graphique est un peu plus lisible et montre une vraie concentration de la richesse sur le dernier centile alors que les neuf autres centiles sont compris entre 1 et 8 milliards.Cependant, il y a encore énormément de données abbérantes dans C10 qui nous pousse à isoler ce dernier centil afin d'avoir une répartition encore plus fine de ces 264 milliardaires.
</details>
...
```{r}
#On va regarder la distribution des 10% les plus riches en les repartissant en centile
top_ten_dt <- dt_centile %>%
  filter(centile =="C10") %>%
  arrange(finalWorth) %>%
  mutate(
    centile = ntile(finalWorth, 10),
...
top_ten_dt
top_ten_cent_plot <- top_ten_dt %>% ggplot(aes(x=factor(centile), y=finalWorth)) +
geom_boxplot()+
...
Finalement à l'intérieur même des 10% les plus fortunés, les inégalités de richesse restent flagrantes. C10 est le box qui montre la plus grande disparité de données alors les neuf autres restent homogènes en leur seins. Mais on note enfin une vraie différence à l'œil nu pour les neuf premiers centiles. On ne pouvait pas déceler cela si facilement avec le C10 du graphique du dessus.  Les écarts vont de 8 à 43 milliards. 
</details>
...
Il y la masse des petits, et aussi, nouveaux milliardaires : ils se situent juste après le seuil de 1 milliard entre C01 et C05 sur la première datavisualisation en centile. 
...
Q15 - Quelles sont les différents types d'industrie qui sont les plus représentés ? 
<details>
<summary>Question et problème étudié</summary>
</details>
