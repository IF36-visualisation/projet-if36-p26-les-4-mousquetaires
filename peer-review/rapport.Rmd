...
Pour conclure, les milliardaires sont répartis dans les pays de l'Ouest et dans les BRICS, idem pour les nationalités, mais ils sont concentrés dans les pays développés, notamment les paradis fiscaux. Les 
...
<summary>Visualisation</summary>
```{r echo=FALSE}
# Attribution d'un numéro différent tous les 660 données (2640/4) pour faire une répartition en quartile
num_quartile <-c("Q4", "Q3", "Q2", "Q1" )
dt_quart <- df %>% select(finalWorth) %>% mutate(quartile = rep(num_quartile, each = 660, length.out = n())) %>% mutate(finalWorth = finalWorth/1000)

# Box plot pour tester la répartition
distr_rich_quart_plot <- dt_quart %>% ggplot(aes(x=factor(quartile), y=finalWorth)) +
geom_boxplot() +
  labs(title ="Distribution de la richesse des milliardaires en quartiles",
       x="Quartile",
       y="Milliards") 
...
dt_centile<- df %>% select(finalWorth) %>% mutate(centile = rep(num_centile, each = 2640/10, length.out = n())) %>%
...
```
```{r}
#On va enlever le dernier centile pour bien voir la répartition des milliardaires afin de vérifier s'il n'y a pas des choses que nous empêche de voir le dernier centile
new_dt_centile <- dt_centile %>%filter(centile !="C10") %>% mutate(centile = rep(num_centile, each = 2376/10, length.out = n()))

new_distr_rich_cent_plot <- new_dt_centile %>% ggplot(aes(x=factor(centile), y=finalWorth)) +
geom_boxplot()+
  labs(title ="Distribution de la richesse des milliardaires en centile, excluant les 10% les plus riches",
       x="Centile",
       y="Milliards")
new_distr_rich_cent_plot
```

```{r}
#On va regarder la distribution des 10% les plus riches en les repartissant en centile
top_ten_dt <- dt_centile %>%filter(centile =="C10") %>% mutate(centile = rep(num_centile, each = 264/10, length.out = n()))

top_ten_cent_plot <- top_ten_dt %>% ggplot(aes(x=factor(centile), y=finalWorth)) +
geom_boxplot()+
  labs(title ="Distribution de la richesse des 10% des milliardaires les plus riches en centile",
       x="Centile",
       y="Milliards")
top_ten_cent_plot
...
```{r}
#On va regarder la distribution des 10% les plus riches en les repartissant en quantile
top_ten_quart_dt <- dt_centile %>%filter(centile =="C10") %>% mutate(centile = rep(num_quartile, each = 264/4, length.out = n())) 

top_ten_quart_plot <- top_ten_quart_dt %>% ggplot(aes(x=factor(centile), y=finalWorth)) +
geom_boxplot()+
  labs(title ="Distribution de la richesse des 10% des milliardaires les plus riches en quartile",
       x="Quartile",
       y="Milliards")
top_ten_quart_plot
```
